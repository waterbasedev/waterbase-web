...
import { isDescendant, findDocfromId, handleNewItem } from '../utils/document-helper';
...
const Sidebar = ({
    ...
    const handleDrop = (e, dropTarget) => {
        ...
        if (docToUpdate) {
            ...
            if (docToUpdate.type === 'folder' && docToUpdate.children) {
                if (isDescendant(docToUpdate, dropTarget, (id) => findDocfromId(id, documents))) {
        ...
        return (
            <>
                {folders.map(folder => (
                    ...
                    >
                        <div className="folder-item" onClick={() => setSelectedItem(folder)}>
    ...
    return (
        <div className="side-panel">
            ...
            <button className="new-folder-button" onClick={() => handleNewItem(documents, setDocuments, 'folder')}>
                <Plus size={20} />
                New Folder
            </button>

            <button className="new-doc-button" onClick={() => handleNewItem(documents, setDocuments, 'document')}>
                <Plus size={20} />
                New Document
            </button>
        </div>
